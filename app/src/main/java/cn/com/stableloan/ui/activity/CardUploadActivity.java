                    ...
                    AppApplication.getUploadManager().put(path, null, token, new UpCompletionHandler() {
                        ...
                        public void complete(String key, ResponseInfo info, JSONObject response) {
                            if (info.isOK()) {
                                ...
                                Glide.with(CardUploadActivity.this).load(path).apply(options).into(fiv);
                                LogUtils.i("onActivityResult", selectList.size());

                                fiv.setOnClickListener(new View.OnClickListener() {
                                    @Override
                                    public void onClick(View v) {
                                        PictureSelector.create(CardUploadActivity.this).externalPicturePreview(selectList.size() - 1, selectList);
                                    }
                                });

                            } else {
                                ToastUtils.showToast(CardUploadActivity.this, "上传失败");
                            }
                        }
                    }, uploadOptions);
                    ...
                    public void onSuccess(String s, Call call, Response response) {
                        try {
                            ...
                            if ("1".equals(isSuccess)) {
                                ...
                                ToastUtils.showToast(CardUploadActivity.this, msg);
                            } else {
                                String msg = object.getString("msg");
                                ToastUtils.showToast(CardUploadActivity.this, msg);
